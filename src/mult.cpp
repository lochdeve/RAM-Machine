...
Mult::Mult(std::string nombreinstruccion, std::string operando) {
...
void Mult::ejecutar(Memoria& memoria, unsigned long& program_counter,
                    bool debug) {
  switch (direccionamiento_) {
    case DIRECTO: {
      if (debug) {
        std::cout << "Instruccion: " << program_counter << " Se multiplica "
                  << memoria.get_memoria()[operando_] << " por "
    ...
    case INDIRECTO: {
      if (debug) {
        std::cout << "Instruccion: " << program_counter << " Se multiplica "
                  ...
                  << " por " << memoria.get_memoria()[0] << std::endl;
      }
      memoria.set_memoria(
    ...
    case INMEDIATO: {
      if (debug) {
        std::cout << "Instruccion: " << program_counter << " Se multiplica "
                  << operando_ << " por " << memoria.get_memoria()[0]
                  << std::endl;
      }
...
std::string Mult::get_nombre() { return nombre_instruccion_; }

std::string Mult::get_operando() { return std::to_string(operando_); }